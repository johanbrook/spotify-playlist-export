module.exports = (function() {
  ...
  var Helpers = {
    ...
    bindEvents: function(eventMap) {
      ...
      Object.keys(eventMap).forEach(function(elementAndEvent) {
        var func = eventMap[elementAndEvent],
            element = elementAndEvent.split(' ')[1],
            evt = elementAndEvent.split(' ')[0]
        ...
        if(element === 'window') {
          ...
          $(document).on(evt, element, func.bind(app))
        }
      })
    },
    ...
    calculateExpireDateFromToday: function(offsetInSeconds) {
      var now = new Date()
      return now.setSeconds(now.getSeconds() + offsetInSeconds)
    },
